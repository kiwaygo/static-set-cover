...
constexpr std::size_t flagImpl() {
    if constexpr (sizeof...(tElements) == tI) {
        return 0;
    } else if (std::is_same_v<std::tuple_element_t<tI, std::tuple<tElements...>>, tElement>) {
...
constexpr std::size_t commonality(const std::size_t aSet0, const std::size_t aSet1) {
...
template <std::size_t tSet>
constexpr auto toCanonicalList() {
    if constexpr (tSet == 0) {
...
struct Universe {
    ...
    static constexpr std::size_t sSet = (... | flag<tSetElements, tElements...>());
    ...
    using KPerm = std::conditional_t<hasRepeats<tKPermElements...>(), NotList, List<tKPermElements...>>;
