...
auto makeEvalType(std::tuple<tEvaluables...>) -> std::tuple<typename tEvaluables::Type...>;

template <typename tUniverse, typename... tCovers>
struct Evaluator {
...
protected:
    ...
    static auto sparseEval(tArgs&&... aArgs) {
        if constexpr (tEvalSet == 0) {
            ...
            constexpr std::size_t bestEvalSet = argmaxCommonality<tEvalSet, sEvalSet<tCovers>...>();
            using BestCover = typename CoverByEvalSet::template Find<std::integral_constant<std::size_t, bestEvalSet>>;
            auto src = BestCover()(std::forward<tArgs>(aArgs)...);
            auto tgt = sparseEval<tEvalSet & (~bestEvalSet)>(std::forward<tArgs>(aArgs)...);
            replace(tgt, src, std::make_index_sequence<std::tuple_size_v<decltype(src)>>{}, typename BestCover::EvalList{});
            return tgt;
        }
    }
